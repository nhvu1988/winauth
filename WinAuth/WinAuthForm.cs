...
namespace WinAuth
{
    public partial class WinAuthForm : ResourceForm
    {
            ...
            for (int i = 1; i < args.Length; i++)
            {
                ...
                if (arg[0] == '-')
                {
                    switch (arg)
                    {
                        ...
                        case "--log":
                            i++;
                            FieldInfo fi = typeof(LogLevel).GetField(args[i],
                            ...
                            var target = NLog.LogManager.Configuration.AllTargets.Where(t => t.Name == null)
                                .FirstOrDefault();
                            if (target != null)
                            {
                                LogManager.Configuration.LoggingRules.Add(
                                    new NLog.Config.LoggingRule("*", loglevel, target));
                            }
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(uri.UserInfo) == false)
                    {
                        ...
                        webproxy.Credentials =
                            new NetworkCredential(auth[0], (auth.Length > 1 ? auth[1] : string.Empty));
                    }
            ...
            }).ContinueWith((configTask) =>
            {
                ...
                else if (ex is Exception)
                {
                    if (ErrorDialog(this, strings.UnknownError + ": " + ex.Message, ex,
                            MessageBoxButtons.RetryCancel) == System.Windows.Forms.DialogResult.Cancel)
                    {
                        this.Close();
                        return;
                    }
        ...
        private void InitializeForm()
        {
            ...
            _listoffset = new Rectangle(authenticatorList.Left, authenticatorList.Top,
                (this.Width - authenticatorList.Width), (this.Height - authenticatorList.Height));

            // set the shadow type (change in config for compatibility)
            try
            {
                MetroFormShadowType shadow =
                    (MetroFormShadowType) Enum.Parse(typeof(MetroFormShadowType), this.Config.ShadowType, true);
            ...
            if (this.Config.Position.IsEmpty == false)
            {
                ...
                if ((this.Config.Position.X + this.Width) >= v.Left && this.Config.Position.X < v.Width &&
                    this.Config.Position.Y > v.Top)
                {
                    try
                    {
            ...
            Exception ex)
        {
            ...
            var auth = this.Config.Cast<WinAuthAuthenticator>().Where(a =>
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            else if (m.Msg == WinAPI.WM_DEVICECHANGE)
            {
                ...
                if (wParam == WinAPI.DBT_DEVICEARRIVAL)
                {
                    ...
                    {
                        if (this.Config == null ||
                            (this.Config.PasswordType &
                             (Authenticator.PasswordTypes.YubiKeySlot1 | Authenticator.PasswordTypes.YubiKeySlot2)) !=
                            0 && m_yubis.Count == 0)
                        {
                            m_yubis = HIDDevice.GetAllDevices(YubiKey.VENDOR_ID);
                            if (m_yubis.Count != 0)
                ...
                {
                    if (this.Config != null && (this.Config.PasswordType &
                                                (Authenticator.PasswordTypes.YubiKeySlot1 |
                                                 Authenticator.PasswordTypes.YubiKeySlot2)) != 0)
                    {
        ...
        private void WinAuthForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.Config != null && this.Config.AutoSize == false &&
                (this.Config.Width != this.Width || this.Config.Height != this.Height))
            {
            ...
            if (registeredauth != null)
            {
                ...
                else
                {
                    throw new NotImplementedException(strings.AuthenticatorNotImplemented + ": " +
                                                      registeredauth.AuthenticatorType.ToString());
                }
        ...
        private void NotifyMenuOnOpened(object sender, EventArgs eventArgs)
        {
            IsMenuStripOpen = true;
        }

        private void NotifyMenuOnClosing(object sender, ToolStripDropDownClosingEventArgs toolStripDropDownClosingEventArgs)
        {
            IsMenuStripOpen = false;
        }

        bool IsNotifyMenuScrolling = false;

        private void NotifyMenuOnMouseWheel(object sender, MouseEventArgs e)
        {
            if (IsMenuStripOpen && !IsNotifyMenuScrolling)
            {
                if (!(sender is ContextMenuStrip menuStrip) || menuStrip.Items.Count < 3)
                    return;//nothing to scroll
                bool goUp = e.Delta > 0;
                ToolStripItem scrollItem = menuStrip.GetItemAt(e.X, goUp ? menuStrip.Padding.Top : menuStrip.Height - menuStrip.Padding.Bottom);
                if (scrollItem == null)
                    return;//nothing to scroll

                if (goUp && scrollItem == menuStrip.Items[0])//goes to bottom
                    return;
                    //scrollItem = menuStrip.Items[1];
                else if (!goUp && scrollItem == menuStrip.Items[menuStrip.Items.Count - 1]) //goes to top
                    return;
                    //scrollItem = menuStrip.Items[menuStrip.Items.Count - 2];
                IsNotifyMenuScrolling = true;
                scrollItem.Select();
                SendKeys.SendWait(goUp ? "{UP}" : "{DOWN}");
                SendKeys.Flush();
                IsNotifyMenuScrolling = false;
            }
        }
        ...
        private void changePasswordOptionsMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (form.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
            {
                ...
                do
                {
                    ...
                    this.Config.PasswordType = form.PasswordType;
                    if ((this.Config.PasswordType &
                         (Authenticator.PasswordTypes.YubiKeySlot1 | Authenticator.PasswordTypes.YubiKeySlot2)) != 0 &&
                        form.Yubikey != null)
                    {
                        this.Config.Yubi = form.Yubikey;
                    }
                    ...
                    {
                        var result = WinAuthForm.ConfirmDialog(this, "Decryption test failed. Retry?",
                            MessageBoxButtons.YesNo);
                    ...
                    {
                        var result = WinAuthForm.ConfirmDialog(this, "YubiKey Challenge/Response failed. Retry?",
                            MessageBoxButtons.YesNo);
                        if (result == DialogResult.Yes)
                        {
                            retry = true;
        ...
        private void exportOptionsMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                WinAuthHelper.ExportAuthenticators(this, this.Config, exportform.ExportFile, exportform.Password,
                    exportform.PGPKey);
            }
        }
